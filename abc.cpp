...
void ABC::normalabc(int narg, char **arg)
{
    ...
    for(int shear_step = 0; shear_step < maxstep; shear_step++){
	if (comm->me == 0) fprintf(screen,"ABC STEP: %d \n",shear_step+1);
	if(screen && comm->me == 0) fprintf(screen,"-----------NVE run-----------\n");
	run(1,runarg);
	...
	//record the initial configuration
        ...
        if(shear_step % ndump == 0){
            ...
            sprintf(str,"write_data    data_%d.init",shear_step);  
        ...
        for (int igaus = 0; igaus < maxpf; igaus++){
             ...
             if(igaus == maxpf*0.6){
             	dmin = dmin*0.8;
		ftolerance = ftolerance*1.2;
		ebarrier = ebarrier*0.8;
             }        

             if(igaus == maxpf*0.8){
             	dmin = dmin*0.8;
		ftolerance = ftolerance*1.5;
		ebarrier = ebarrier*0.8;
             }  
        ...
        if(screen && comm->me == 0) fprintf(screen,"Start KMC \n");
         ...
         if(lm_no <= 1){
             ...
             //choose the one with largest energy barrier
             double lowest_energy = 1e10;
             for(int i = 1; i < lm_no; i++){
                 if(energy_steps[i] < lowest_energy){
                     lowest_energy = energy_steps[i];
                     flag_peak = i;
                 }
             }
         }
...
/* ----------------------------------------------------------------------*/
void ABC::run(int narg, char **arg)
{
